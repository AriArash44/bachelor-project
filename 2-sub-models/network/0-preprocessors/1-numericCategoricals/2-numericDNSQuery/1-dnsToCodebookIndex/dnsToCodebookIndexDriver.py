...
VOCAB_PKL = "../0-codebookTraining/0-embedDNS/dns2idx.pkl"
EMB_MODEL_H5 = "../0-codebookTraining/0-embedDNS/dns_embedding_model_trained.h5"
...
class dnsEncodingPipeline:
    def __init__(self, dns2idx: dict):
        self.dns2idx = dns2idx
    ...
    def _safe_lookup(self, dns: str) -> int:
        return self.dns2idx.get(dns, self.dns2idx.get(UNK_TOKEN, 1))
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out = df.copy()
        out["dns_query"] = nearest_idx
        return out
    
def main():
    p = argparse.ArgumentParser(description="Load vocab from pickle + Fit/Transform dns→codebook pipeline")
    subs = p.add_subparsers(dest="mode", required=True)
    fit = subs.add_parser("fit", help="Load dns2idx.pkl, transform train, save pipeline")
    ...
    fit.add_argument("--preproc-pkl", default="dns_pipeline.pkl", help="path of pickle pipeline")
    ...
    tr.add_argument("--preproc-pkl", default="dns_pipeline.pkl", help="path of pickle pipeline")
    args = p.parse_args()
    if args.mode == "fit":
        with open(VOCAB_PKL, "rb") as f:
            dns2idx = pickle.load(f)
        pipeline = dnsEncodingPipeline(dns2idx)
